...
public class HoewonService {
	...
	public void setHoewonInput() {
		...
		while(true) {
			...
			if(vo != null) {
				System.out.println("같은 성명이 존재합니다. 다른이름으로 가입하세요");
			} else {
				break;
			}
		}
		
		System.out.print("회원가입할 나이 입력하세요? ");
		age = scanner.nextInt();
	...
	public void setHoewonUpdate() {
		...
		while(run) {
			...
			try {
				...
				System.out.println("회원번호를 양수로 입력하세요");
				scanner.next(); // scanner.nextInt()로 입력시 예외발생할 경우는 catch구문에서 입력버퍼를 삭제처리한다.
			}
		}
